...
if go and not global.pause{
	...
	switch(state){
        case "ghostfollow":  //move torwards player to give contact damage
			...
			scr_move_entity(point_direction(x,y,global.body.x,global.body.y),0.75);
		...
		case "magego2fire": //make mage go to the nearest unlit fire and light it 
			...
			scr_define_path(self, targetfire);
			path = global.ai_path;
			scr_move_entity(point_direction(x,y,path_get_point_x(path,1),path_get_point_y(path,1)),1);
			
			if(point_distance(x,y,targetfire.x+targetfire.sprite_width/2,targetfire.y+targetfire.sprite_height/2)<=30){
				state = "lightfire";
			}
		break;
		
		case "lightfire": //make mage go to the nearest unlit fire and light it 
