...
if(place_meeting(x,y,obj_ppon)){
    ...
    if(distance_to_object(obj_body) > 128){
        sprite_index = spr_slm_b_w;
        phy_position_x += sign(obj_body.x - x)*e_spd;
        phy_position_y += sign(obj_body.y - y)*e_spd;
    }

    /// Attack
    if(distance_to_object(obj_body) <= 128 && distance_to_object(obj_body) > 32 && atk_cd == false){
        instance_create(x+16,y+16,obj_slm_bullet);
        sprite_index = spr_slm_b_f;
        atk_cd = true;
        alarm[0] = 1.5*room_speed;
    }
    
    ///Run away
    if(distance_to_object(obj_body) <= 48){
        sprite_index = spr_slm_b_w;
        phy_position_x -= sign(obj_body.x - x)*e_spd;
        phy_position_y -= sign(obj_body.y - y)*e_spd;  
    }
    ...
    if(e_hp <= 0){
        global.combat -= 1;
        instance_destroy();
        instance_create(x,y,obj_blue_puff);
        if((irandom(99) + 1) <= 10){
